...
Downloader.prototype.downloadFlv = function(url, start, end, seq) {
  var self = this;
  function _pump(e) {
    return e.read().then(function(res) {
      // n 就是 getReader() 的返回值
      if (!res.done) {
        let chunk = res.value.buffer;
        console.log('downloadFlv', start, end, seq,chunk);
        self.reportData(start, end, seq,chunk);
        return _pump(e);
      } else {
      }
    });
  ...
  fetch(url).then(res => {
    ...
    if (res.ok && res.status >= 200 && res.status <= 299) {
      ...
      if (lengthHeader != null) {
        flvObj._contentLength = parseInt(lengthHeader);
        if (flvObj._contentLength !== 0) {
          if (flvObj._onContentLengthKnown) {
            flvObj._onContentLengthKnown(flvObj._contentLength);
          }
        }
      }

      return _pump.call(this, res.body.getReader());
    }
  });
};
